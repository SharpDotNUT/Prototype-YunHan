...
let data_loaded = false,
  Achievement = []
fetch(
  'https://snap-hutao-meta-data.pages.dev/Genshin/CHS/Achievement.json'
).then((res) =>
  res.json().then((data) => {
    Achievement = data
    data_loaded = true
...
function render() {
  ...
  userAchievements = userAchievements.filter(
    (achievement) => achievement.timestamp !== 0
  )
  ...
  for (let i = 0; i < userAchievements.length; i++) {
    ...
    let achievementDataObject = AchievementIdToAchievementDataObject(
      achievement.id
    )
...
<template>
  ...
  <var-card
    ...
    :key="achievement.timestamp"></var-card>
